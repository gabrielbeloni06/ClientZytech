...
export default function ClientsPage() {
  ...
  const getTypeStyle = (type: string) => {
    switch(type) {
      case 'commerce': return { icon: <ShoppingCart size={14}/>, color: 'text-orange-400 bg-orange-500/10 border-orange-500/20', label: 'Comércio' }
      case 'delivery': return { icon: <Truck size={14}/>, color: 'text-green-400 bg-green-500/10 border-green-500/20', label: 'Delivery' }
      case 'real_estate': return { icon: <Home size={14}/>, color: 'text-indigo-400 bg-indigo-500/10 border-indigo-500/20', label: 'Imobiliária' }
      case 'website': return { icon: <Globe size={14}/>, color: 'text-blue-400 bg-blue-500/10 border-blue-500/20', label: 'Website' }
      case 'automation': return { icon: <Cpu size={14}/>, color: 'text-purple-400 bg-purple-500/10 border-purple-500/20', label: 'Automação' }
        ...
        {isModalOpen && (
            ...
            <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden relative">
                ...
                <div>
                    ...
                    <input 
                    autoFocus type="text" placeholder="Ex: Imobiliária Silva"
                    className="w-full bg-[#050505] border border-white/10 rounded-xl p-3 text-white focus:border-blue-500 focus:bg-white/[0.02] outline-none transition-all placeholder:text-gray-700"
                ...
                <div>
                    ...
                    {[
                        ...
                        className={`flex items-center gap-2 p-3 rounded-xl border text-xs font-bold uppercase transition-all duration-300 ${
                ...
                {newClientType !== 'consulting' && (
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        ...
                        {(newClientType === 'commerce' || newClientType === 'delivery' || newClientType === 'real_estate') && ['ZyStart', 'ZyControl', 'ZyBotAI', 'ZyCore'].map(plan => <PlanButton key={plan} plan={plan} current={newClientPlan} set={setNewClientPlan} />)}
                ...
                <div className="grid grid-cols-2 gap-4 pt-6 border-t border-white/5">
                    <div>
                        ...
                        <div className="relative group">
                            ...
                            <input type="text" placeholder="0,00" className="w-full bg-[#050505] border border-white/10 rounded-xl p-3 pl-9 text-white focus:border-green-500 focus:bg-white/[0.02] outline-none transition-all placeholder:text-gray-700 font-mono"
                    ...
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <select value={newClientCycle} onChange={(e) => setNewClientCycle(e.target.value)} className="w-full bg-[#050505] border border-white/10 rounded-xl p-3 pl-9 text-white focus:border-blue-500 focus:bg-white/[0.02] outline-none appearance-none cursor-pointer hover:bg-white/5 transition-colors">
                ...
                <div className="pt-4 flex justify-end gap-3">
                    ...
                    <button type="submit" disabled={isSaving} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-sm font-bold flex items-center gap-2 transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)] hover:shadow-[0_0_25px_rgba(37,99,235,0.5)] disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-0.5 active:translate-y-0">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/5 pb-6">
            ...
            <div className="flex items-center gap-3">
                <div className="relative hidden md:block">
                    ...
                    className="bg-white/5 border border-white/10 rounded-xl pl-9 pr-4 py-2.5 text-sm text-gray-300 focus:border-blue-500/50 focus:bg-white/10 outline-none w-64 transition-all placeholder:text-gray-600"
                    value={searchTerm}
                ...
                <tbody className="divide-y divide-white/5">
                    ...
                    filteredClients.map((client) => {
                        ...
                        <tr key={client.id} className="hover:bg-white/[0.02] transition-colors group">
                            ...
                            <td className="p-5">
                            <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-300 ${style.color}`}>
