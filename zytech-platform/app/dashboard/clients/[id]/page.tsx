...
import { OverviewTab, SettingsTab, CatalogTab, AppointmentsTab, OrdersTab, NotificationsTab, ChatTab, CategoriesTab } from './tabs'
...
export default function ClientDetailsPage() {
  ...
  const handleOpenProductModal = (productToEdit: any = null) => {
      if (productToEdit) {
          setEditingProduct(productToEdit);
          setProdName(productToEdit.name);
          setProdPrice(String(productToEdit.price));
          setProdCategory(productToEdit.category);
          setProdHasStockLimit(productToEdit.track_stock);
          setProdStockQuantity(String(productToEdit.stock_quantity || ''));
          setProdAddress(productToEdit.address || '');
          setProdNeighborhood(productToEdit.neighborhood || '');
          setProdLink(productToEdit.property_link || '');
          setProdDetails(productToEdit.property_details || '');
      } else {
          setEditingProduct(null);
          setProdName(''); setProdPrice(''); setProdCategory(''); 
          setProdHasStockLimit(false); setProdStockQuantity(''); 
          setProdAddress(''); setProdNeighborhood(''); setProdLink(''); setProdDetails('');
      }
  ...
  async function handleSaveProduct(e: React.FormEvent) { 
    ...
    const productData: any = { 
        ...
        category: prodCategory || (client.business_type === 'real_estate' ? 'Imóvel' : labels.category), 
        is_available: true, 
        track_stock: prodHasStockLimit, 
        stock_quantity: prodHasStockLimit ? parseInt(prodStockQuantity) : null, 
        property_details: prodDetails 
    };

    if (client.business_type === 'real_estate') { 
        productData.address = prodAddress; 
        productData.neighborhood = prodNeighborhood; 
        productData.property_link = prodLink; 
  ...
  async function handleSaveCategories() { 
      ...
      const { error } = await supabase.from('organizations').update({ service_categories: customCategories }).eq('id', client.id); 
      if (error) alert('Erro: ' + error.message); else alert('Categorias atualizadas!'); 
    ...
    <div className="min-h-screen bg-[#020202] text-white font-sans selection:bg-blue-500/30">
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-10 relative z-10 space-y-8">
        ...
        <div className="min-h-[400px]">
            ...
            {activeTab === 'bot_catalog' && <CatalogTab client={client} isRealEstate={isRealEstate} isServiceType={isServiceType} products={products} setIsProductModalOpen={() => handleOpenProductModal(null)} labels={labels} toggleProductStatus={toggleProductStatus} handleDeleteProduct={handleDeleteProduct} handleEditProduct={handleOpenProductModal} />}
            {activeTab === 'categories' && <CategoriesTab customCategories={customCategories} newCategoryInput={newCategoryInput} setNewCategoryInput={setNewCategoryInput} handleAddCategory={handleAddCategory} handleRemoveCategory={handleRemoveCategory} handleSaveCategories={handleSaveCategories} isSavingCategories={isSavingCategories} />}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in">
            <div className="bg-[#0a0a0a] border border-white/10 w-full max-w-sm rounded-2xl shadow-2xl relative overflow-hidden">
                <div className="p-6 space-y-5">
                    <div className="flex justify-between items-center"><h3 className="text-lg font-bold text-white">{editingProduct ? 'Editar ' : 'Adicionar '}{labels.add.replace('Novo ','')}</h3><button onClick={() => setIsProductModalOpen(false)} className="text-gray-500 hover:text-white"><X size={20}/></button></div>
                    <form onSubmit={handleSaveProduct} className="space-y-4">
                        ...
                        <button disabled={isSavingProd} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-xl font-bold shadow-[0_0_15px_rgba(37,99,235,0.3)] transition-all mt-2">{isSavingProd ? 'Salvando...' : 'Salvar Dados'}</button>
                    </form>
                </div>
            </div>
        </div>
