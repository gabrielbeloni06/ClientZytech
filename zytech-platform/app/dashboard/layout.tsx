...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const getNavItems = () => {
    ...
    if (orgId) {
        items.push({ 
            name: businessType === 'delivery' ? 'Meu Delivery' : 'Meus Serviços', 
    ...
    <div className="min-h-screen bg-[#050505] text-gray-100 flex font-sans selection:bg-blue-500/30">
      ...
      <aside className="hidden md:flex flex-col w-72 border-r border-white/5 bg-[#0a0a0a] relative overflow-hidden">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto custom-scrollbar relative z-10">
          <p className="px-4 py-2 text-[10px] font-bold text-gray-600 uppercase tracking-widest">Menu Principal</p>
          {navItems.map((item) => {
        ...
        <div className="p-4 border-t border-white/5 relative z-10">
          <div className="bg-white/5 rounded-xl p-1 mb-3">
             <div className="flex items-center gap-3 p-2">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-blue-600 to-purple-600 flex items-center justify-center text-xs font-bold shrink-0 shadow-lg">
                  {orgName.charAt(0)}
                </div>
                <div className="overflow-hidden">
                  <p className="text-xs font-bold text-white truncate">{orgName}</p>
                  <p className="text-[10px] text-gray-400 truncate">Sessão Ativa</p>
                </div>
             </div>
          </div>
          <button 
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-black relative">
        ...
        {isMobileMenuOpen && (
            <div className="md:hidden absolute top-[70px] left-0 w-full bg-[#0a0a0a] border-b border-white/10 p-4 space-y-2 z-40 shadow-2xl backdrop-blur-xl animate-in slide-in-from-top-2">
                ...
                <button onClick={handleLogout} className="flex items-center gap-3 px-4 py-3 w-full text-red-400 hover:bg-red-500/10 rounded-lg">
